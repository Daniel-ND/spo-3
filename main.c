...
//        execute_client(argc, argv);
        ...
        if (argc < 4) {
       	   char *boilerplate = "/usr/bin/wget -q -p --header=\"User-Agent: Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_0) AppleWebKit/600.1.17 (KHTML, like Gecko) Version/8.0 Safari/600.1.17\" -e robots=off ";
       	   strcat(arr, boilerplate);
       	   strcat(arr, argv[argc - 1]);
    	   system(arr);
    	} else {
           char *boilerplate = "/usr/bin/wget -q ";
           strcat(arr, boilerplate);
       	   strcat(arr, argv[argc - 1]);
    	   system(arr);
        }
